...
class Buffer
{
     ...
     * @param  bufferHandle A pointer of computeBuffer with float4 that has been
     * generated with Unity (see function GetNativeBufferPtr
    ...
    /**
     * Unregister the buffer in CUDA, this has to be override because it depends
     * on the graphics api
     */
     ...
     * return an array of T* defined on device memory and which can be edited in
     * cuda
     */
    template <typename T> UNITY_INTERFACE_EXPORT int mapResources(T **vertexPtr)
    {
        // map resource
        CUDA_CHECK_RETURN(
        ...
        CUDA_CHECK_RETURN(cudaGraphicsResourceGetMappedPointer(
            (void **)vertexPtr, &numBytes, _graphicsResource));
